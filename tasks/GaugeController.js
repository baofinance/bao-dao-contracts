...
task("bao:GaugeController:addGauge", "Add a gauge to the controller.")
  ...
  .addParam(
    "type",
    "The index of the gauge type (previously setup) on the controller"
  )

  .setAction(
    async (taskArgs, { network, ethers, deployments, getNamedAccounts }) => {
      ...
      const { address: gcAddress, abi: gcAbi } = await deployments.get(
        "GaugeController"
      );
      ...
      const { address: gaugeAddress } = await deployments.get(
        `LiquidityGaugeV3_${taskArgs.gauge}`
      );
      tx = await controller["add_gauge(address,int128,uint256)"](
        gaugeAddress,
        parseInt(taskArgs.type),
        taskArgs.weight
      );
...
task("bao:GaugeController:addGaugeType", "Add a gauge type to the controller.")
  ...
  .setAction(
    async (taskArgs, { network, ethers, deployments, getNamedAccounts }) => {
      ...
      let tx;
      tx = await controller["add_type(string,uint256)"](
        taskArgs.name,
        taskArgs.weight
      );
      await tx.wait();
...
task(
  "bao:GaugeController:changeOwner",
  "Change the admin of the GaugeController contract."
)
