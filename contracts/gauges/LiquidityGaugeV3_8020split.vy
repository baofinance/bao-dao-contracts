# @version 0.2.12
"""
@title Liquidity Gauge v3 - 80/20 split
...
rewards_split: public(address)


@external
def __init__(_lp_token: address, _minter: address, _admin: address, _rewards_split: address):
    """
    ...
    bao_token: address = Minter(_minter).token()
    controller: address = Minter(_minter).controller()

    self.lp_token = _lp_token
    self.minter = _minter
    self.admin = _admin
    self.bao_token = bao_token
    self.controller = controller
    self.voting_escrow = Controller(controller).voting_escrow()

    self.period_timestamp[0] = block.timestamp
    self.inflation_rate = BAO20(bao_token).rate()
    self.future_epoch_time = BAO20(bao_token).future_epoch_time_write()

    self.rewards_split = _rewards_split
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of BAO token per-user.
            Effectively it calculates working balances to apply amplification
            of BAO production by BAO
    ...
    if _user != ZERO_ADDRESS:
        ...
        for i in range(MAX_REWARDS):
            ...
            if total_claimable > 0:
                ...
                twenty_percent: uint256 = total_claimable / 5
                eighty_percent: uint256 = total_claimable - twenty_percent
                if _claim:
                    response1: Bytes[32] = raw_call(
                        token,
                        concat(
                            method_id("transfer(address,uint256)"),
                            convert(self.rewards_split, bytes32),
                            convert(twenty_percent, bytes32),
                        ),
                        max_outsize=32,
                    )
                    if len(response1) != 0:
                        assert convert(response1, bool)
                    response2: Bytes[32] = raw_call(
                        token,
                        concat(
                            method_id("transfer(address,uint256)"),
                            convert(receiver, bytes32),
                            convert(eighty_percent, bytes32),
                        ),
                        max_outsize=32,
                    )
                    if len(response2) != 0:
                        assert convert(response2, bool)
...
def _checkpoint(addr: address):
    ...
    if prev_future_epoch >= _period_time:
        _token: address = self.bao_token
        self.future_epoch_time = BAO20(_token).future_epoch_time_write()
        new_rate = BAO20(_token).rate()
...
def reward_contract() -> address:
    """
    @notice Address of the reward contract providing non-BAO incentives for this gauge
...
def set_killed(_is_killed: bool):
    ...
    @dev When killed, the gauge always yields a rate of 0 and so cannot mint BAO
