...
def minter_setup(accounts, mock_lp_token, token, minter, gauge_controller, gauge_v3):
    ...
    for acct in accounts[1:4]:
        ...
        mock_lp_token.approve(gauge_v3, 1e18, {"from": acct})


@given(st_duration=strategy("uint[3]", min_value=WEEK, max_value=MONTH, unique=True))
@settings(max_examples=30)
def test_duration(accounts, chain, gauge_v3, minter, token, st_duration):
    accts = accounts[1:]
    chain.sleep(7 * 86400)

    deposit_time = []
    for i in range(3):
        gauge_v3.deposit(10 ** 18, {"from": accts[i]})
    ...
    for i in range(3):
        ...
        gauge_v3.withdraw(10 ** 18, {"from": accts[i]})
...
@settings(max_examples=30)
def test_amounts(accounts, chain, gauge_v3, minter, token, st_amounts):
    accts = accounts[1:]

    deposit_time = []
    for i in range(3):
        gauge_v3.deposit(st_amounts[i], {"from": accts[i]})
        deposit_time.append(chain[-1].timestamp)

    chain.sleep(MONTH)
    balances = []
    for i in range(3):
        gauge_v3.withdraw(st_amounts[i], {"from": accts[i]})
