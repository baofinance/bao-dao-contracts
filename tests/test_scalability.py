...
def gauges(LiquidityGaugeV3, accounts, gauge_controller, mock_lp_token, minter, setup):
    # deploy `GAUGE_COUNT` liquidity gauges and return them as a list
    gauges = []
    for i in range(GAUGE_COUNT):
        contract = LiquidityGaugeV3.deploy(mock_lp_token, minter, accounts[0], {"from": accounts[0]})
