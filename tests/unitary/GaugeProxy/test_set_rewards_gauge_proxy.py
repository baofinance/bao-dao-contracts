...
def setup(alice, gauge_proxy, gauge_v3, mock_lp_token):
    ...
    mock_lp_token.approve(gauge_v3, 10 ** 18, {"from": alice})
    gauge_v3.deposit(10 ** 18, {"from": alice})


def test_set_rewards(alice, gauge_proxy, gauge_v3, reward_contract, coin_reward):
    sigs = [
    ...
    gauge_proxy.set_rewards(
        gauge_v3, reward_contract, sigs, [coin_reward] + [ZERO_ADDRESS] * 7, {"from": alice}
    )
...
def test_only_ownership_admin(accounts, gauge_proxy, gauge_v3, reward_contract, idx):

    with brownie.reverts("Access denied"):
        gauge_proxy.set_rewards(
            gauge_v3, reward_contract, "0x00", [ZERO_ADDRESS] * 8, {"from": accounts[idx]}
        )
